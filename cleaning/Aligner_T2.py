...
def main():
    ...
    input_uwrf = xr.open_dataset(f'/D4/data/gvaillant/uwrf-cleaned/T2/uwrf_cleaned_{str(month).zfill(2)}_T2.nc')
    input_nam = xr.open_dataset(f'/D4/data/gvaillant/NAM-cleaned/T2/nam_cleaned_{str(month).zfill(2)}_TMP_2maboveground.nc')
    output_dir = '/D4/data/gvaillant/NAM-aligned'

    aligned_ds = regrid_nam_like_uwrf(input_nam, input_uwrf)

    # Save to NetCDF
    output_path = os.path.join(output_dir, f'nam_aligned_{str(month).zfill(2)}_T2.nc')
