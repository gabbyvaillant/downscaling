...
def regrid_nam_like_uwrf(nam_data, uwrf_data, target_shape=(30, 30)):
    ...
    nam_data = nam_data.rename({'UGRD_10maboveground': 'U10'})
    nam_data = nam_data.rename({'VGRD_10maboveground': 'V10'})
...
def main():
    ...
    input_uwrf = xr.open_dataset(f'/D4/data/gvaillant/uwrf-cleaned/WS/uwrf_cleaned_{str(month).zfill(2)}_U10_V10.nc')
    input_nam = xr.open_dataset(f'/D4/data/gvaillant/NAM-cleaned/WS/nam_cleaned_{str(month).zfill(2)}_WS.nc')
    output_dir = '/D4/data/gvaillant/NAM-aligned'

    aligned_ds = regrid_nam_like_uwrf(input_nam, input_uwrf)

    # Save to NetCDF
    output_path = os.path.join(output_dir, f'nam_aligned_{str(month).zfill(2)}_WS.nc')
